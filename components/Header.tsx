    ...
    <div>
      ...
      >
        <div className="rounded-[12px] bg-[#f7f8f9] px-3 py-3">
          {nav.mega ? (
            <>
              {nav.mega.sections.map((section, i) => (
                <div key={section.title} className="mb-3 last:mb-0">
                  <div className={`flex items-center gap-3 px-4 pb-2 ${i === 0 ? 'pt-1' : 'pt-2.5'}`}>
                    <span className="h-px min-w-[12px] flex-1 bg-[#0c1d22]/12" aria-hidden />
                    <p className="shrink-0 font-sans text-[11px] font-bold uppercase tracking-[0.14em] text-[#0c1d22]/40">
                      {section.title}
                    </p>
                    <span className="h-px min-w-[12px] flex-1 bg-[#0c1d22]/12" aria-hidden />
                  </div>
